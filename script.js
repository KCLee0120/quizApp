...
continue_btn.onclick = () => {
  if(document.getElementById("class").value=="" || document.getElementById("classNo").value==""){
    alert("請先選擇班別及學號");
  }else{
    for(var k=0; k<students.length; k++){
      if(students[k].numb==document.getElementById("classNo").value){
        if(students[k].pw==document.getElementById("pw").value){
          info_box.classList.remove("activeInfo"); //hide info box
...
function showQuetions(index) {
  ...
  let queImg_tag =
    '<img src="Q' + questions[index].numb + '.png" class="que_img"></img>';
  let que_tag =
    "<span>" +
...
function showResult() {
  ...
  if (userScore > 3) {
    ...
    let scoreTag =
      "<span>你答對了 " + userScore + " 題！你已經充分掌握弧長公式！</span>";
    scoreText.innerHTML = scoreTag; //adding new span tag inside score_Text
  } else if (userScore > 1) {
    // if user scored more than 1
    let scoreTag =
      "<span>你答對了 " + userScore + " 題！你已經初步掌握弧長公式！</span>";
...
function startTimerLine(time) {
  counterLine = setInterval(timer, 94);
  function timer() {
